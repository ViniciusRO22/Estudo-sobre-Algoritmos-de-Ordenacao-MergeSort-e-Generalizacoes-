...
public class MergeSortListaDinamica{
        ...
        try(BufferedWriter csvWriter = new BufferedWriter(new FileWriter("/home/siratuti/AEDS/testeJava/Java/data/Output/output.dat", true))){
            ...
            for(int t = 0; t < tamanhos.length; t++){
                ...
                for(int i = 0; i < 10; i++){
                    ...
                    ArrayList<Float> ratingsLista = new ArrayList<>();

                    try(BufferedReader br = new BufferedReader(new FileReader(csvFile))){
                        String line;
                        br.readLine(); 

                        while((line = br.readLine()) != null && ratingsLista.size() < tamanho){
                            String[] parts = line.split(",");
                            
                            if(parts.length >= 3){
                                try{
                                    ratingsLista.add(Float.parseFloat(parts[2])); 
                                } 
                    ...
                    float[] ratingsArray = new float[ratingsLista.size()];
                    for(int j = 0; j < ratingsLista.size(); j++){
                        ratingsArray[j] = ratingsLista.get(j);
                    }

                    ratingsArray = mergeSort(ratingsArray);

                    ratingsLista.clear();
                    for(float rating : ratingsArray){
                        ratingsLista.add(rating);
                    }
                ...
                csvWriter.write(String.format("%d,lista,dinamica,%.6f,%d%n", 
